...
template <typename T, size_t N>
[[maybe_unused]] void to_json(Json::json &j, const Array<T, N> &value) {
  j = value.value;
}

template <typename T, size_t N>
[[maybe_unused]] void from_json(const Json::json &j, Array<T, N> &value) {
  for (int i = 0; i < N; i++) {
...
template <typename T, size_t N>
std::ostream &operator<<(std::ostream &os, Array<T, N> t) {
...
template <typename T, size_t N>
struct is_array<infra::Array<T, N>> : public std::true_type {};

template <typename V, size_t N, typename T>
struct is_array_of<infra::Array<V, N>, T> {
