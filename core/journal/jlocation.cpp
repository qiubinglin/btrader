...
std::filesystem::path get_root_dir(const std::filesystem::path &root, enums::RunMode m,
    ...
    if (iter == map_env.end()) {
        throw std::runtime_error(
            fmt::format("unrecognized mode: {}, init root_ as mode::LIVE", static_cast<int8_t>(m)));
    }
...
std::string JLocator::layout_file(const JLocationSPtr &location, enums::layout layout, const std::string &name) const {
    auto path = std::filesystem::path(layout_dir(location, layout)) /
                fmt::format("{}.{}", name, enums::get_layout_name(layout));
    return path.string();
}
